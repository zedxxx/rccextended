...
void showHelp(const QString &argv0, const QString &error)
{
    ...
    fprintf(stderr, "Usage: %s  [options] <inputs>\n\n"
        ...
        "  -reverse             Output all resources from binary (*.rcc) files in\n"
        "                       current directiry. Also it generate qrc files and\n"
        "                       make.bat file for repack rasorces back to binary.\n"
        "                       NOT OFFICIAL\n"
...
int runRcc(int argc, char *argv[])
{
    ...
    for (int i = 1; i < args.count() && errorMsg.isEmpty(); i++) {
        ...
        if (args[i][0] == QLatin1Char('-')) {   // option
            ...
            } else if (opt == QLatin1String("-reverse")) {
                RCCReverseLib r;
                r.rccReverse(QDir("./"));
                return 0;
            } else {
